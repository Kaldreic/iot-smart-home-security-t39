...
re-run before it credits a candidate, is the read-matched control.
...
each column within a benchmark is in bold.}
\label{tab:headlines}
\centering
...
\addlinespace
Resilience at scale          & \textsc{AirBugCatcher}                & 0.266 & 0.632 & 0.52 & 40\\
{\footnotesize 15,000 bugs, 75,000 campaigns} & \textsc{AirBugCatcher} $+$ 1 confirmation & 0.359 & 0.090 & 0.92 & 342\\
                             & RDD                                   & \textbf{0.824} & \textbf{0.000} & \textbf{0.21} & 142\\
\bottomrule
\end{tabular*}
\end{table*}
...
Through the full pipeline (a synthesised fuzz trace, a crash-grouping front end
shared by both tools, a backward window per bug, then either minimiser), RDD
...
Over the eight runs RDD spans $[0.76, 0.91]$ and the baseline $[0.67, 0.81]$,
...
reproduction must now survive both the channel and exact matching twice. RDD
produced no false credit in these 336 campaigns, in the 180 truth-table campaigns
on A--F, in the 30 on the suppressor, or in the 75,000 synthetic campaigns of
Section~\ref{ssec:eval-scale}; the corresponding 95\,\% upper bounds on its false
credit rate are 0.9\,\%, 1.7\,\%, 10\,\% and 0.004\,\%. The tool's property is the
$\alpha$-bounded rate of Section~\ref{sec:our_approach}; zero is the measured outcome.

On the real suppressor bug, where a later packet hides the crash, the baseline
recovers 0.875 with 0.125 false credit and RDD recovers every campaign with none;
...
seed-finder (Section~\ref{ssec:eval-levers}).
...
genuine / false credit; reads are given in the text.}
\label{tab:noise}
\centering
\footnotesize
...
about 48. RDD's results are almost invariant to the two models because the
...
recovers more at higher cost; where it does neither, plain enumeration with one
confirming re-run is the better tool.
...
RDD averages 0.824 genuine recovery with no false credit; the baseline averages
...
Each campaign draws its own noise stream, so the bootstrap intervals over bugs
are meaningful: RDD $[0.820, 0.829]$, baseline $[0.261, 0.272]$. They quantify
sampling error over our generator, not agreement with real bugs. RDD's reads are
roughly 142 per campaign against the baseline's 40.
...
recipe falls to roughly 0.2 to 0.35, inside the indifference zone $(p_0, p_1) =
...
runs), so the baseline reads 0.771 here against 0.759 above and 0.867 over thirty
runs; the per-run baseline range is $[0.67, 1.00]$, and only the identity and
minimiser deltas fall outside it.
...
controller; the channel is a Gilbert--Elliott burst process with miss and phantom
rates taken from a corpus of flaky software tests, not from wireless measurements,
and the crash-report variation is a model with rates we chose (backtrace
...
one fifth of RDD's reads. The read-matched control shows that one confirming
...
truncated test's forced ``no'' contributes most of RDD's deep-noise loss. Six
bugs, one stack, one CVE and one synthetic fault mechanism bound what the numbers
can say.
