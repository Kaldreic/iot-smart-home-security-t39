...
redundant tests. \textsc{PMA} is the closest in aim; it cites noise and
...
Mousavi~\cite{quantumdd2024} reduce quantum-program regressions with
property-based tests whose verdict is a fixed-sample statistical check, not a
sequential, bounded-error decision on a real channel.
...
the closest to RDD in workflow, but the distinction lies in the oracle. U-Fuzz walks
...
RDD builds on these concepts rather than competing with them, and changes three
things: bounded enumeration becomes \texttt{ddmin}, one attempt per candidate becomes
a statistical oracle, and the identity layer differs: \textsc{AirBugCatcher} matches crashes by exact source-line trace,
...
a set budget while controlling both error rates. Applying the SPRT to the reproduction oracle brings the false-credit rate under the
$\alpha$ bound where its preconditions hold; across the synthetic sweep it fell from the
baseline's 0.632 to $1.3\times10^{-5}$. Using a statistical test as an oracle is not new: Guderlei and
...
\subsection{Large language models (LLMs) as testing oracles}
...
separate triage step. The closest in purpose is GPTrace~\cite{gptrace2026}, which
