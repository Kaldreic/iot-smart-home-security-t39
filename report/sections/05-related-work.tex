...
and commercial tools. While each area is well developed individually, RDD is unique
in combining them through adaptive minimisation guided by a statistical
...
Assessment (\textsc{PMA})~\cite{pma2025} is the first to depart from this,
modelling the search space's \emph{monotonicity} probabilistically to avoid
redundant tests. \textsc{PMA} is the closest related work, but it only accelerates
\texttt{ddmin} under structural (deterministic) non-monotonicity and does not
\emph{stabilise} the verdict or use an
oracle model with sequential tests or bounded error. All these methods assume a
...
\textsc{SweynTooth}~\cite{sweyntooth2020} identified twelve BLE vulnerabilities
across seven vendors, and \textsc{BrakTooth}~\cite{braktooth2022} found sixteen in
Bluetooth Classic. Both projects conclude at discovery, requiring analysts to
...
RDD's closest counterpart, but the distinction lies in the oracle. U-Fuzz uses a
single backward replay per candidate and records any remaining instability as
anomalies, while RDD employs a sequential, bounded-error verdict.
...
to a small fixed size and manages non-determinism by repeating trials within a set
budget, rather than stabilising the verdict. In its study, a plain replay baseline,
repeated five times to average out the medium, reproduced only 13 to 23 of 190
crashes~\cite{airbugcatcher2024}. % AirBugCatcher RQ5: replay baseline, 13-23/190 over 5 trials
RDD builds upon these concepts rather than competing with them. Three improvements
have been noted: bounded enumeration is replaced by \texttt{ddmin}, and time-budget
repetition is replaced by a statistical oracle. The third improvement is in the
identity layer: while \textsc{AirBugCatcher} matches crashes by exact source-line or
memory-offset, RDD uses a noise-tolerant two-tier cascade.
...
0.638 to 0.000 in all stress tests, a level not achieved in the flaky-test
literature. Using a statistical test as an oracle is not new: Guderlei and
Mayer~\cite{statoracle2007} evaluate random-output programs against a distribution,
but they do so as a batch decision offline, whereas RDD makes sequential,
bounded-error verdicts during the reduction process.
...
LLM-integrated systems~\cite{ddllm2026} with semantic markers, again on replayable
models without a statistical test. Ensemble frameworks differ:
...
leveraging \textsc{BrakTooth} research~\cite{keysightiot,braktooth2022}. Both tools
...
only on local instrumented software where results are repeatable. A market review
...
\subsection{The unoccupied cell}
...
monotonicity but does not provide a bounded-error verdict. \textsc{DDOR} and
LLM-system reducers operate on replayable targets. The quantum reducer uses a
fixed-sample check. RDD occupies this unique position.
