...
These methods depend on assumptions about the test, stated in the original
work. \texttt{ddmin} treats the test as a function of its input, so the same
input always gives the same answer, and its efficiency and minimality rest on
failures being \emph{monotone}: if a set of packets causes a crash, adding
packets does not stop it. The algorithm tolerates an ``unresolved'' outcome at the
cost of more tests, but not an answer that flips between calls. Channel noise
breaks exactly that. If a test fails to
