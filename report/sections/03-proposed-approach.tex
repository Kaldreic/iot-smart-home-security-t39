...
\emph{Gilbert--Elliott} burst channel~\cite{gilbert1960,elliott1963}, a two-state
...
\emph{phantom} crash). The miss and phantom rates come from a corpus of flaky
software tests~\cite{flakytests2014}, not from wireless measurements. In every
...
and $p_1$, the \emph{indifference zone}, is decided by no sequential test with a
bounded error, a limitation intrinsic to the method that the deep-noise regime of
...
$p_0$ sits above the modelled phantom rate and $p_1$ near the modelled reproduce
rate, so both preconditions hold in the simulator by choice of parameters. Two
...
are discarded, not counted as misses. Given the committed judge verdicts, every
...
crash arrives with different addresses, a truncated backtrace or interleaved log
lines, so an exact match fails. RDD uses a cost-gated cascade in the manner of
...
matcher saw. In evaluation it recovered every same-bug pair the matcher had
abstained on (57 of 57) and also accepted every different-bug pair that reached
it (40 of 40). % l3_judge_eval.json
...
Standard \texttt{ddmin}~\cite{ddmin2002} assumes monotonicity: if a set of packets
crashes the device, adding packets does not stop the crash. A \emph{suppressor}
...
At the end each excluded packet is added back once: if adding it makes the crash
...
RDD credits a recipe only after a second, uncached sequential test with a tighter
$\beta$ on the same noisy channel. For a given non-reproducing recipe both the
...
\subsection{Positioning: a SWOT view}\label{ssec:swot}
...
(Section~\ref{sec:evaluation}); RDD costs four to five times the baseline's
device reads; the judge does not discriminate on the pairs it receives; the
